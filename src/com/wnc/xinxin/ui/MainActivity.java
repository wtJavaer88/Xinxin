...
public class MainActivity extends Activity implements OnClickListener, UncaughtExceptionHandler
{
	EditText memoET;
	...
	private void initView()
	{
		...
		findViewById(R.id.imgview_add_fs).setOnClickListener(new PicAddClickListener());
	}
		...
		builder.setItems(this.picMenu, new DialogInterface.OnClickListener()
		{
			...
			public void onClick(DialogInterface arg0, int arg1)
			{
				if (arg1 == 0)
				{
					...
					{
						mPhotoFile = new File(mediaDir, BasicDateUtil.getCurrentDateTimeString() + ".jpg");
						mPhotoFile.delete();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (requestCode == this.LOAD_IMAGE_RESULT && resultCode == RESULT_OK && null != data)
		{
			...
			String scaledImgPath = (MyAppParams.getInstance().getMediaPath() + System.currentTimeMillis() + "_s") + ".jpg";
			if (ImageCompressUtil.transImageToMaxSize(picturePath, scaledImgPath, 1024, 1024, 60))
			{
				// 压缩成功就代表已经备份了
				System.out.println("压缩成功! " + scaledImgPath);
		...
		if (requestCode == this.VOICE_RESULT && data != null)
		{
			try
			{
				...
				if (amrFile != null)
				{
					...
					backupAndInsertMemo(mediaDir, amrFile.getAbsolutePath(), ".amr");
				}
			}
			catch (Exception e)
			{
	...
	private void changePicZone(String imgPath)
	{
		...
		if (imgPath.endsWith(".jpg"))
		{
			...
			curimgView.setImageDrawable(Drawable.createFromPath(mediaDir + imgPath));
		}
		...
		if (latest_imgView == curimgView)
		{
			...
			AbsoluteLayout.LayoutParams params = new AbsoluteLayout.LayoutParams(300, 300, (300 + padding) * (childCount % 3), (300 + padding) * (childCount / 3));
		...
		mTag.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View arg0)
			{
				if (mTag.getCurrentTextColor() == MainActivity.this.TAG_SEL_COLOR)
				{
					...
					MainActivity.this.selTagList.remove(mTag.getText().toString());
					// To do disselect
				}
				else
				{
			...
			public boolean onLongClick(View v)
			{
				...
				add_tag_dialg_ok.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						...
						{
							Log.v("==", mTag.getText().toString() + "==" + MainActivity.this.mTagList.get(j).toString());
							if (mTag == MainActivity.this.mTagList.get(j))
							{
								try
								{
									...
									{
										MainActivity.this.tag_vessel.removeView(mTag);
