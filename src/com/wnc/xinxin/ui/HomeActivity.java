...
public class HomeActivity extends Activity implements UncaughtExceptionHandler
{
	...
	private FootStepInfo getFsSample(int index)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			media.setAbsulute_path(MyAppParams.getInstance().getWorkPath() + i + ".jpg");
			medias.add(media);
		}
	...
	private AbsoluteLayout getPicZone(RelativeLayout inner_rl, FootStepInfo footStepInfo)
	{
		...
		for (int i = 0; i < footStepInfo.getMedias().size(); i++)
		{
			...
			imgView.setImageDrawable(Drawable.createFromPath(footStepInfo.getMedias().get(i).getAbsulute_path()));
	...
	OnClickListener fsViewClickListener = new OnClickListener()
	{
		...
		public void onClick(View arg0)
		{
			...
			startActivity(new Intent(HomeActivity.this, MediaViewActivity.class).putExtra("index", imgTag.getIndex()).putStringArrayListExtra("medias", imgs));
		}
	};
