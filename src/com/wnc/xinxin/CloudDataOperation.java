...
public class CloudDataOperation
{
		...
		try
		{
			...
			client.post(urlpath, params, new AsyncHttpResponseHandler()
			{
				...
				public void onSuccess(int arg0, Header[] arg1, byte[] arg2)
				{
					...
					{
						up_flag = true;
						System.out.println("耗时:" + (System.currentTimeMillis() - stime) / 1000);
					}
				}
			});
		}
		...
		try
		{
			...
			client.post(urlpath, params, new AsyncHttpResponseHandler()
			{
				...
				public void onSuccess(int statusCode, Header[] headers, byte[] response)
				{
					...
					System.out.println("耗时:" + (System.currentTimeMillis() - stime) / 1000);
					if (BasicStringUtil.isNullString(string))
					{
						return;
					}
					...
					try
					{
						String savepath = MyAppParams.getInstance().getWorkPath() + "/test/" + BasicFileUtil.getFileName(string);
						String folder = MyAppParams.getInstance().getWorkPath() + "/test/" + BasicFileUtil.getFileName(string).replaceAll("\\.\\S+", "") + File.separator;
						UrlPicDownloader.download(string, savepath);
						ZipUtils.upZipFile(new File(savepath), folder);
						// 复制文件到media目录
						for (File f : new File(folder).listFiles())
						{
							BasicFileUtil.CopyFile(f.getAbsolutePath(), MyAppParams.getInstance().getMediaPath() + f.getName());
						}

						List<String> readFrom = FileOp.readFrom(folder + "data.json", "UTF-8");
						if (readFrom.size() > 0)
						{
							JSONObject jObject = JSONObject.parseObject(readFrom.get(0));
							...
							if (parseArray.size() > 0)
							{
								for (FootStepInfo footStepInfo : parseArray)
								{
									...
									if (fsService.existFs(footStepInfo.getUuid(), footStepInfo.getUpdateTime()))
									{
										System.out.println("该记录没有任何更新, 本机已存在");
									}
									else if (fsService.existFs(footStepInfo.getUuid()))
									{
										fsService.update(footStepInfo);
									}
									else
										fsService.save(footStepInfo);
								}
							}
						}
						down_flag = true;
	...
	private File freshDataToFile() throws IOException
	{
		...
		final String dataFile = MyAppParams.getInstance().getWorkPath() + "data.json";
		final String zipFilePath = MyAppParams.getInstance().getWorkPath() + System.currentTimeMillis() + ".zip";
		FsService fsService = new FsService();
		List<FootStepInfo> findAll = fsService.findAllNeedUpload(lastUploadTime);
		if (findAll.size() == 0)
		{
			return null;
		}
		...
		BasicFileUtil.writeFileString(dataFile, jsonObject.toString(), null, false);
		...
		for (FootStepInfo footStepInfo : findAll)
		{
			...
			if (medias != null)
			{
				...
				{
					String media_fullpath = media.getMediaFullpath();
					if (FileTypeUtil.isVideoFile(media_fullpath.replace(".jpg", "")) || FileTypeUtil.isVoiceFile(media_fullpath.replace(".jpg", "")))
					{
