...
public class FsDao
{
	...
	public synchronized static void insertFs(FootStepInfo fsInfo)
	{
		...
		{
			openDatabase();
			database.execSQL("INSERT INTO FOOTSTEPS(fs_desc,create_time,update_time) VALUES (?,?,?)", new Object[] { fsInfo.getDesc(), fsInfo.getCreate_time(), fsInfo.getUpdate_time() });
		}
		catch (Exception e)
		{
	...
	public synchronized static void insertComplicateFs(FootStepInfo fsInfo, List<FsMedia> medias, List<Integer> tagIDs)
	{
		try
		{
			openDatabase();
			...
			for (int i = 0; i < medias.size(); i++)
			{
				...
				database.execSQL("INSERT INTO FS_MEDIAS(fs_id,mediapath_id,media_name,media_size,media_type,sn,create_time) VALUES (?,?,?,?,?,?,?)", new Object[] { fs_id, fsMedia.getMediapath_id(), fsMedia.getMedia_name(), fsMedia.getMedia_size(), fsMedia.getMedia_type(), i + 1,
	...
	{
		Cursor c = database.rawQuery("SELECT MAX(ID) CUR_ID FROM FOOTSTEPS", null);
		c.moveToFirst();
		while (!c.isAfterLast())
		{
	...
	private static List<FsMedia> findMedias(FootStepInfo fs)
	{
		...
		{
			openDatabase();
			Cursor c = database.rawQuery("SELECT * FROM FS_MEDIAS WHERE FS_ID=" + fs.getId(), null);
			c.moveToFirst();
