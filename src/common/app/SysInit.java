...
public class SysInit
{
	...
	public static void init(Activity context)
	{
		...
		if (isFirstRun())
		{
			...
			{
				BasicFileUtil.CopyFile(MyAppParams.XINXIN_DB, MyAppParams.getInstance().getBackupDbPath() + System.currentTimeMillis() + ".db");
		...
		if (!BasicFileUtil.isExistFile(MyAppParams.XINXIN_DB))
		{
			try
			{
				BasicFileUtil.writeFileByte(MyAppParams.XINXIN_DB, IOUtils.toByteArray(context.getAssets().open("xinxin.db")));
			}
			catch (IOException e)
			{
				e.printStackTrace();
		...
		SQLiteOpenHelper myHelper = new SQLiteHelperOfOpen(context, MyAppParams.XINXIN_DB, null, 1);
